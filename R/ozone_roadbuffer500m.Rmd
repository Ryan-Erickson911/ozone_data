...
subtitle: "Variable: Total Length of Roads w/ in 500m"
...
plot(den_projected)
plot(o3_500m, col="red", add=TRUE)
plot(roads_projected, col="green", add=TRUE)

polys.sub <- roads_projected[!is.na(sp::over(o3_500m[2,], geometry(roads_projected))),] 
plot(polys.sub)
...
for(i in seq_along(cols)) {
  var_name = cols[i]
  v = autofitVariogram(reformulate("1",var_name),year_o3)
  vgms[[i]] = v
}
# vgms
...
for(i in seq_along(cols)) {
  var_name = cols[i]
  k = autoKrige(reformulate("1",var_name),year_o3,grd, model = c("Ste","Sph","Mat","Exp","Nug","Gau","Lin"))
  k.cv = autoKrige.cv(reformulate("1",var_name),year_o3,model = c("Ste","Sph","Mat","Exp","Nug","Gau","Lin"))
