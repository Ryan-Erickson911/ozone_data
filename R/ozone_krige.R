...
o3 = read_csv("final_data/colorado_o3_pivot_mthly_wide.csv") # the "../" bit of code is only so the .R file will run for the .Rmd file
# head(o3)
...
o3 = as.data.frame(o3) %>%
  ...
  dplyr::mutate_at(vars(-site_name, -long, -lat), ~ . * 1000) %>% # changes units to parts per billion
  dplyr::select(-"2022-11-30", -"2022-10-31")
...
# Remove missing obs
o3 = o3[(o3$site_name %in% remove_NA[which(remove_NA$sumNA==0),]$site_name),]
# project stations
...
co_projected = as(co_co, "Spatial")
# den_projected = as(den_co, "Spatial")
grd = SpatialPixels(SpatialPoints(makegrid(co_projected)))
